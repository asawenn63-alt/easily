  ...
  for (const resolvedScene of resolvedSceneGraph.scenes) {
    ...
    for (const resolvedNode of resolvedScene.nodes) {
      ...
      if (isTextBearing(authoredNode) && resolvedNode.resolvedTypography) {
        ...
        // This estimate does not have access to the browser's actual font
        // metrics. Permit less than a quarter line of measurement variance,
        // but keep rejecting genuinely truncated text.
        const measurementTolerancePx = Math.max(2, (Number(resolvedNode.resolvedTypography.lineHeightPx) || 0) * 0.25);
        if (requiredHeight > resolvedNode.bounds.height + measurementTolerancePx) {
          conflicts.push(issue("text_box_overflow", "Text cannot fit inside its authored bounds at the resolved typography size.", {
            sceneId: resolvedScene.sceneId,
            nodeId: resolvedNode.nodeId,
            details: { requiredHeight, availableHeight: resolvedNode.bounds.height, width: resolvedNode.bounds.width, measurementTolerancePx },
          }));
        }
      }
    ...
    for (const relation of authoredScene.relations || []) {
      if (relation.kind !== "aspect" || relation.strength !== "required" || relation.value?.unit !== "ratio") continue;
      const expectedRatio = Number(relation.value.value);
      ...
      for (const nodeId of relation.subjects || []) {
        ...
        if (!resolvedNode?.visible || !resolvedNode.bounds?.height) continue;
        const actualRatio = resolvedNode.bounds.width / resolvedNode.bounds.height;
        if (Math.abs(actualRatio - expectedRatio) / expectedRatio <= 0.015) continue;
        conflicts.push(issue("authored_aspect_violated", "Resolved bounds do not preserve the authored required aspect ratio.", {
          sceneId: resolvedScene.sceneId,
          nodeId,
          relationId: relation.id,
          details: { expectedRatio, actualRatio, bounds: resolvedNode.bounds },
        }));
      }
    }
 ...
 */
export function compileLayout({ sceneGraph, resolvedSceneGraph }) {
  const conflicts = [
    ...schemaIssues(validateUnresolvedSceneGraph(sceneGraph), "free_scene"),
    ...schemaIssues(validateResolvedSceneGraph(resolvedSceneGraph, { sceneGraph }), "resolved_scene_graph"),
  ];
  ...
  const receipt = {
    ...
    orderedSceneIds: resolvedSceneGraph.scenes.map((scene) => scene.sceneId),
    orderedNodeIds: resolvedSceneGraph.scenes.flatMap((scene) => scene.orderedNodeIds),
  };
